function w = smoothed_l1_regression(train_D, lambda)
    ...
    eta_0 = 0.5;                  % Step size for gradient descent
    eta = eta_0;
    
    % Criteria for the loop to end
    max_n_iterations = 1e4;
    eta_min = 1e-30;              % Step size at which to cut off and end
    cutoff_grad1 = 1e-1;          % Cut to the lower tau
    cutoff_grad2 = 1e-3;          % End
    ...
    iter = 1;
    while iter < max_n_iterations && mod_grad(iter) > cutoff_grad2 && eta > eta_min
